...
verify(application.includes('ECOSYSTEM_APPLICATIONS.map(application =>'), 'Every catalog application is shown on the public homepage.');
verify(application.includes("const QuestionStudioApp = lazy(() => import('./components/QuestionStudioApp'))"), 'The standalone Question Studio route is lazy-loaded from the ecosystem homepage.');
...
verify(main.includes('<QuestionStudioLibraryPortal />'), 'The teacher portal mounts the standalone Question Studio library card.');
...
verify(ECOSYSTEM_APPLICATIONS.some(application => application.name === 'Trắc nghiệm 10 dạng'), 'The new standalone Question Studio application is registered.');
...
console.info(`Thirteen-application catalog with standalone Question Studio: ${checks} checks passed.`);
