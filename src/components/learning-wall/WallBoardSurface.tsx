  ...
  return <div className={`lw-app ${board ? 'lw-board-open' : ''}`}>
    ...
    <main className="lw-main" style={board ? { backgroundColor: background.color, backgroundImage: background.pattern, backgroundSize: background.pattern.startsWith('radial') ? '18px 18px' : 'cover' } : undefined}>
      ...
      {board ? <><section className="lw-board-heading"><span className="lw-board-emoji">{board.wallIcon || '📚'}</span><div><p className="lw-eyebrow">{guest ? 'CÙNG CHIA SẺ VÀ KHÁM PHÁ' : board.author || displayName}</p><h1>{board.title}</h1>{board.wallDescription && <p className="lw-description">{board.wallDescription}</p>}<p className="lw-board-count">{currentPosts.length} bài đăng · {sections.length} cột{board.wallArchived ? ' · Đã lưu trữ' : ''}</p></div></section>
        {guest && <div className={`lw-guest-banner ${props.guestCanPost ? 'is-write' : ''}`} role="status"><div><strong>{props.guestCanPost ? 'Đây là liên kết nộp bài của đúng lớp này' : 'Bảng đang ở chế độ chỉ xem'}</strong><p>{props.guestCanPost ? 'Bài gửi sẽ vào hộp chờ để giáo viên kiểm tra trước khi công khai.' : 'Giáo viên chưa mở nhận bài trên liên kết này.'}</p></div>{props.guestCanPost && <button className="lw-button lw-primary" onClick={() => openPost()}><Send size={17} />Nộp bài vào lớp</button>}</div>}
    ...
    {panel === 'share' && board && <WallModal title="Công khai bảng cho học sinh" onClose={close} error={actionError}><div className="lw-form"><div className="lw-share-intro"><Globe size={23} /><div><h3>Liên kết gắn với đúng bảng “{board.title}”</h3><p>Học sinh chỉ nhìn thấy bài của bảng này. Điểm, nhận xét riêng và dữ liệu lớp không được đưa lên liên kết.</p></div></div><p className="lw-help">Chọn một chế độ bên dưới. Khi bật nhận bài, học sinh nhập tên, nội dung, ảnh hoặc liên kết; bài sẽ chờ thầy duyệt trước khi xuất hiện công khai.</p>
      <div className="lw-share-modes"><button className="lw-share-mode" disabled={busy} onClick={() => void run(() => actions.share(board, 'read'), 'Đã công khai bảng ở chế độ chỉ xem.', false)}><LockKeyhole size={18} /><span><strong>Chỉ xem</strong><small>Học sinh chỉ đọc bài đã công khai.</small></span></button><button className="lw-share-mode primary" disabled={busy} onClick={() => void run(() => actions.share(board, 'write'), 'Đã mở nhận bài cho đúng lớp; bài mới luôn chờ duyệt.', false)}><Send size={18} /><span><strong>Cho phép học sinh nộp bài</strong><small>Học sinh gửi bài vào hộp chờ của bảng này.</small></span></button></div>
      {props.shareUrl && <><div className="lw-qr"><QrCode value={props.shareUrl} /><p>{props.sharePermission === 'write' ? 'Đang nhận bài chờ duyệt — đúng bảng này' : 'Đang chia sẻ chỉ xem'}</p></div><label>Liên kết gửi cho học sinh<input readOnly value={props.shareUrl} onFocus={event => event.target.select()} /></label><button className="lw-button" onClick={() => void run(() => navigator.clipboard.writeText(props.shareUrl!), 'Đã sao chép liên kết nộp bài.', false)}><Copy size={17} />Sao chép liên kết</button><button className="lw-button lw-danger" disabled={busy} onClick={() => void run(() => actions.revoke(board), 'Đã tắt liên kết của bảng này.', false)}><LockKeyhole size={17} />Tắt liên kết</button></>}
