...
interface LuckyDrawProps {
  onBack: () => void;
  categories?: { id: string; title: string; parentId?: string }[];
  allStudents?: { id: string; classId: string; name: string }[];
...
export default function LuckyDraw({ onBack, categories = [], allStudents = [], initialMode = 'wheel' }: LuckyDrawProps) {
        ...
        try {
          ...
          if (results.landmarks && results.landmarks.length > 0) {
            ...
            if (cursorRef.current && containerRef.current) {
              ...
              const smoothing = 0.15; // Lower value = smoother but more lag
              ...
              if (cardEl) {
                ...
                if (isFist && drawModeRef.current === 'wheel') {
                  if (!isSpinningRef.current && !showWinnerModalRef.current) {
  ...
  return (
    <div ref={containerRef} className="flex flex-col h-full bg-slate-50 relative">
      ...
      <div 
        ...
        style={{ top: -16, left: -16, transition: 'transform 0.1s ease-out' }}
      />
            ...
            {categories && categories.length > 0 && (
              <div className="mb-4">
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-sm bg-slate-50"
                >
