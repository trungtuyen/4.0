...
export default function AdminDashboard({ onLogout, teachers, setTeachers }: AdminDashboardProps) {
  ...
  const handleResetPassword = (id: string) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: (passwordValue?: string) => {
        if (!passwordValue) {
  ...
  const handleToggleStatus = (id: string, currentStatus: 'active' | 'inactive') => {
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        setTeachers(teachers.map(t => t.id === id ? { ...t, status: currentStatus === 'active' ? 'inactive' : 'active' } : t));
        setConfirmDialog(prev => ({ ...prev, isOpen: false }));
      }
    });
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {activeTab === 'teachers' ? (
          ...
          <>
            <header className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between">
              <h1 className="text-2xl font-bold text-slate-800">Thư viện tương tác</h1>
            </header>
            <div className="flex-1 overflow-auto p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <h3 className="text-lg font-bold text-slate-800 mb-2">Vòng quay may mắn</h3>
                  <p className="text-slate-500 text-sm">Vòng quay ngẫu nhiên tạo sự hứng thú trong các hoạt động trên lớp.</p>
                </div>
                ...
                >
                  ...
                  <h3 className="text-lg font-bold text-slate-800 mb-2">Bốc thẻ</h3>
                  <p className="text-slate-500 text-sm">Công cụ bốc thẻ ngẫu nhiên với hiệu ứng lật thẻ và điều khiển bằng cử chỉ.</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow cursor-pointer group">
                  ...
                  <h3 className="text-lg font-bold text-slate-800 mb-2">Tương tác thẻ Plicker</h3>
                  <p className="text-slate-500 text-sm">Công cụ đánh giá trắc nghiệm nhanh sử dụng thẻ mã QR.</p>
                </div>
                ...
                >
                  ...
                  <h3 className="text-lg font-bold text-slate-800 mb-2">Tường học tập</h3>
                  <p className="text-slate-500 text-sm">Quản lý lớp học, học sinh, chấm điểm và tổ chức thư mục học tập.</p>
                </div>
              </div>
            </div>
          </>
