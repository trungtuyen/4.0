...
export default function AdminDashboard({ onLogout, teachers, setTeachers, currentUser }: AdminDashboardProps) {
  const [activeTab, setActiveTab] = useState<'teachers' | 'library' | 'chatbot'>(currentUser === 'admin' ? 'teachers' : 'library');
  const [activeLibraryView, setActiveLibraryView] = useState<'main' | 'learning-wall' | 'lucky-draw' | 'lucky-draw-cards' | 'plicker' | 'head-shake-game'>('main');
        ...
        <aside className="w-64 bg-slate-900 text-white flex flex-col hidden md:flex shrink-0">
          ...
          <nav className="flex-1 p-4 space-y-2">
            ...
            >
              ...
              <span className="font-medium">AI Phân tích tâm lý</span>
            </button>
          </nav>
      ...
      {!(activeTab === 'library' && activeLibraryView !== 'main') && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex items-center justify-around p-2 z-50">
          ...
          <button 
            onClick={() => setActiveTab('chatbot')}
            className={`flex flex-col items-center gap-1 p-2 transition-colors ${
              activeTab === 'chatbot' ? 'text-indigo-600' : 'text-slate-400'
            }`}
          >
            <MessageSquare className="w-6 h-6" />
            <span className="text-[10px] font-medium">AI Tâm lý</span>
          </button>
        </nav>
      )}
